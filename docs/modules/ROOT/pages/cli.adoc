...
The Mapping Suite SDK provides a command-line interface (CLI) that allows users to interact with mapping packages directly from the terminal. The CLI offers conversion (between package versions v1/v2/v3/v3L) and validation of mapping packages from folders, archives, and GitHub.
...
* `convert`: Convert mapping packages between versions (v1/v2 → v3, v1/v2/v3 → v3L). In-place conversion. Use `--to-version` (v3 or v3L) and `--from-version` (v1, v2, or v3). Subcommands: `from-package`, `from-folder`.
* `validate`: Validate mapping packages from folder, archive, or GitHub.
...
mssdk convert --to-version v3L --from-version v3 from-folder /path/to/folder
----
