"""
Unit tests for MappingPackageV2Repository.

Tests cover CRUD operations and MongoDB-specific behavior for V2 mapping packages.
"""
...
from mapping_suite_sdk.mapping_package_v2.adapters.mp_v2_repository import (
    MappingPackageV2Repository,
    PACKAGE_VERSION,
...
def test_create_with_success_non_existing_element(
    ...
    repository = MappingPackageV2Repository(
        mongo_client=mongo_client,
        database_name="test_db"
    )

    result_id = repository.create(dummy_mapping_package_v2_model)
    stored_result = repository.collection.find_one({"_id": dummy_mapping_package_v2_model.id})

    assert result_id == dummy_mapping_package_v2_model.id
    assert stored_result is not None
    assert stored_result["package_version"] == PACKAGE_VERSION
    assert stored_result["_id"] == dummy_mapping_package_v2_model.id

    # Remove MongoDB-specific fields and validate
    stored_result.pop("_id", None)
    stored_result.pop("package_version", None)
    stored_model = MappingPackageV2.model_validate(stored_result)
    assert stored_model.metadata.identifier == dummy_mapping_package_v2_model.metadata.identifier
...
def test_create_fails_on_creating_existing_element(
    ...
    repository.create(dummy_mapping_package_v2_model)
...
def test_read_with_success_existing_element(
    ...
    repository.create(dummy_mapping_package_v2_model)
    stored_model = repository.read(dummy_mapping_package_v2_model.id)

    assert stored_model is not None
    assert stored_model.id == dummy_mapping_package_v2_model.id
    assert stored_model.metadata.identifier == dummy_mapping_package_v2_model.metadata.identifier
...
def test_read_fails_on_non_existing_element(
    ...
    with pytest.raises(ModelNotFoundError) as exc_info:
        repository.read(dummy_mapping_package_v2_model.id)

    assert f"Mapping package with ID {dummy_mapping_package_v2_model.id} not found" in str(exc_info.value)
...
def test_read_many_with_success_without_filters(
    ...
    package1 = dummy_mapping_package_v2_model.model_copy(deep=True)
    package1.metadata.identifier = "package_1"
    package2 = dummy_mapping_package_v2_model.model_copy(deep=True)
    package2.metadata.identifier = "package_2"
    package3 = dummy_mapping_package_v2_model.model_copy(deep=True)
...
def test_update_with_success_existing_element(
    ...
    # Update the package
    updated_package = dummy_mapping_package_v2_model.model_copy(deep=True)
...
def test_update_fails_on_non_existing_element(
    ...
    with pytest.raises(ModelNotFoundError):
        repository.update(dummy_mapping_package_v2_model)
...
def test_delete_with_success_existing_element(
    ...
    repository.delete(dummy_mapping_package_v2_model.id)

    assert repository.collection.find_one({"_id": dummy_mapping_package_v2_model.id}) is None
...
def test_repository_uses_custom_collection_name(mongo_client: mongomock.MongoClient):
    ...
    custom_collection = "custom_v2_collection"
    repository = MappingPackageV2Repository(
        mongo_client=mongo_client,
        database_name="test_db",
