...
class MPV3LightweightStructuralValidationException(MPValidationException): pass


class MPV3LightweightHashValidationException(MPValidationException): pass


class MPV3LightweightHashValidationStep(MPValidationStepABC):
    """
    ...
    def validate(self,
        ...
        mp_hasher = MappingPackageV3LightweightHasher(mapping_package=mapping_package, hasher=hasher)
        generated_hash: str = mp_hasher.hash()

        try:
            assert generated_hash == mapping_package.metadata.mapping_suite_hash_digest
        except AssertionError:
            raise MPV3LightweightHashValidationException(
...
@final
@traced_class
class MappingPackageV3LightweightValidator:
    """
    ...
    def __init__(self, validation_chain: Optional[MPValidationStepABC] = None):
        self.validation_chain = validation_chain or MPV3LightweightHashValidationStep()

    def validate(self, mapping_package: MappingPackageV3Lightweight) -> Literal[True] | NoReturn:
