...
def test_technical_mapping_collection_asset_serialiser_rejects_mock_asset(tmp_path: Path):
    """Test that TechnicalMappingCollectionAssetSerialiser rejects MagicMock assets."""
    serialiser = TechnicalMappingCollectionAssetSerialiser()
    ...
    assert "Expected TechnicalMappingCollectionAsset" in str(excinfo.value)
    assert "MagicMock" in str(excinfo.value) or "got" in str(excinfo.value)


def test_technical_mapping_collection_asset_serialiser_rejects_mock_path(tmp_path: Path):
    """Test that TechnicalMappingCollectionAssetSerialiser rejects assets with mock paths."""
    serialiser = TechnicalMappingCollectionAssetSerialiser()
    
    # Create a valid asset but with a mock path
    # Use object.__setattr__ to bypass Pydantic validation
    technical_mapping = TechnicalMappingCollectionAsset(
        path=Path("transformation/mappings"),
        files=[]
    )
    object.__setattr__(technical_mapping, 'path', MagicMock())  # Replace path with mock

    with pytest.raises(TypeError) as excinfo:
        serialiser.serialise(tmp_path, technical_mapping)
    
    assert "Expected path to be str or Path" in str(excinfo.value)
    assert "MagicMock" in str(excinfo.value) or "got" in str(excinfo.value)


def test_technical_mapping_collection_asset_serialiser_rejects_wrong_type(tmp_path: Path):
    """Test that TechnicalMappingCollectionAssetSerialiser rejects wrong asset types."""
    serialiser = TechnicalMappingCollectionAssetSerialiser()
    
    # Pass a VocabularyMappingCollectionAsset instead of TechnicalMappingCollectionAsset
    vocabulary_mapping = VocabularyMappingCollectionAsset(
        path=Path("transformation/resources"),
        files=[]
    )
    ...
    with pytest.raises(TypeError) as excinfo:
        serialiser.serialise(tmp_path, vocabulary_mapping)
    
    assert "Expected TechnicalMappingCollectionAsset" in str(excinfo.value)
    assert "VocabularyMappingCollectionAsset" in str(excinfo.value)


def test_vocabulary_mapping_collection_asset_serialiser_rejects_mock_asset(tmp_path: Path):
    """Test that VocabularyMappingCollectionAssetSerialiser rejects MagicMock assets."""
    serialiser = VocabularyMappingCollectionAssetSerialiser()
    mock_asset = MagicMock()
    ...
    assert "Expected VocabularyMappingCollectionAsset" in str(excinfo.value)
    assert "MagicMock" in str(excinfo.value) or "got" in str(excinfo.value)


def test_vocabulary_mapping_collection_asset_serialiser_rejects_mock_path(tmp_path: Path):
    """Test that VocabularyMappingCollectionAssetSerialiser rejects assets with mock paths."""
    serialiser = VocabularyMappingCollectionAssetSerialiser()
    ...
    object.__setattr__(vocabulary_mapping, 'path', MagicMock())  # Replace path with mock
...
def test_vocabulary_mapping_collection_asset_serialiser_rejects_wrong_type(tmp_path: Path):
    """Test that VocabularyMappingCollectionAssetSerialiser rejects wrong asset types."""
    serialiser = VocabularyMappingCollectionAssetSerialiser()
    
    # Pass a TechnicalMappingCollectionAsset instead of VocabularyMappingCollectionAsset
    technical_mapping = TechnicalMappingCollectionAsset(
        path=Path("transformation/mappings"),
        files=[]
    )
    ...
    assert "Expected VocabularyMappingCollectionAsset" in str(excinfo.value)
    assert "TechnicalMappingCollectionAsset" in str(excinfo.value)
