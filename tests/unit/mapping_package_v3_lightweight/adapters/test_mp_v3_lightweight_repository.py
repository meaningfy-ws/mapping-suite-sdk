"""
Unit tests for MappingPackageV3LightweightRepository.

Tests cover CRUD operations and MongoDB-specific behavior for V3 lightweight mapping packages.
"""
...
from mapping_suite_sdk.mapping_package_v3.adapters.mp_v3_lightweight_repository import (
    MappingPackageV3LightweightRepository,
    PACKAGE_VERSION,
...
@pytest.fixture
def fixture_mapping_package_v3_lightweight_model(
    fixture_mapping_package_v3_model
) -> MappingPackageV3Lightweight:
    """Fixture for V3 lightweight mapping package model."""
    return convert_mapping_package_v3_to_v3_lightweight(fixture_mapping_package_v3_model)


def test_create_with_success_non_existing_element(
    mongo_client: mongomock.MongoClient,
    fixture_mapping_package_v3_lightweight_model: MappingPackageV3Lightweight
):
    """Test creating a new mapping package in MongoDB."""
    repository = MappingPackageV3LightweightRepository(
        mongo_client=mongo_client,
        database_name="test_db"
    )

    result_id = repository.create(fixture_mapping_package_v3_lightweight_model)
    stored_result = repository.collection.find_one({"_id": fixture_mapping_package_v3_lightweight_model.id})

    assert result_id == fixture_mapping_package_v3_lightweight_model.id
    assert stored_result is not None
    assert stored_result["package_version"] == PACKAGE_VERSION
    assert stored_result["_id"] == fixture_mapping_package_v3_lightweight_model.id

    # Remove MongoDB-specific fields and validate
    stored_result.pop("_id", None)
    stored_result.pop("package_version", None)
    stored_model = MappingPackageV3Lightweight.model_validate(stored_result)
    assert stored_model.metadata.id == fixture_mapping_package_v3_lightweight_model.metadata.id
...
def test_create_fails_on_creating_existing_element(
    ...
    repository.create(fixture_mapping_package_v3_lightweight_model)
...
def test_read_with_success_existing_element(
    ...
    repository.create(fixture_mapping_package_v3_lightweight_model)
    stored_model = repository.read(fixture_mapping_package_v3_lightweight_model.id)

    assert stored_model is not None
    assert stored_model.id == fixture_mapping_package_v3_lightweight_model.id
    assert stored_model.metadata.id == fixture_mapping_package_v3_lightweight_model.metadata.id
...
def test_read_fails_on_non_existing_element(
    ...
    with pytest.raises(ModelNotFoundError) as exc_info:
        repository.read(fixture_mapping_package_v3_lightweight_model.id)

    assert f"Mapping package with ID {fixture_mapping_package_v3_lightweight_model.id} not found" in str(exc_info.value)
...
def test_read_many_with_success_without_filters(
    ...
    package1 = fixture_mapping_package_v3_lightweight_model.model_copy(deep=True)
    package1.metadata.id = "package_1"
    package2 = fixture_mapping_package_v3_lightweight_model.model_copy(deep=True)
    package2.metadata.id = "package_2"
    package3 = fixture_mapping_package_v3_lightweight_model.model_copy(deep=True)
...
def test_update_with_success_existing_element(
    ...
    # Update the package
    updated_package = fixture_mapping_package_v3_lightweight_model.model_copy(deep=True)
...
def test_update_fails_on_non_existing_element(
    ...
    with pytest.raises(ModelNotFoundError):
        repository.update(fixture_mapping_package_v3_lightweight_model)
...
def test_delete_with_success_existing_element(
    ...
    repository.delete(fixture_mapping_package_v3_lightweight_model.id)

    assert repository.collection.find_one({"_id": fixture_mapping_package_v3_lightweight_model.id}) is None
...
def test_repository_uses_custom_collection_name(mongo_client: mongomock.MongoClient):
    ...
    custom_collection = "custom_v3lightweight_collection"
    repository = MappingPackageV3LightweightRepository(
        mongo_client=mongo_client,
        database_name="test_db",
