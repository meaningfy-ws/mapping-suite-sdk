...
Load and optionally convert/validate programmatically (version auto-detected):
...
# Serialisation: use version-specific serialisers (e.g. serialise_mapping_package_v2_to_folder)
# or convert in-place via CLI: mssdk convert --to-version v3 --from-version v2 from-package ...
----
...
add_span_processor_to_mssdk_tracer_provider(SimpleSpanProcessor(ConsoleSpanExporter()))
----
