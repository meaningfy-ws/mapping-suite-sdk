...
def _copy_context_jsonld_to_package(mapping_package_folder_path: Path, converted_package) -> None:
    ...
    # Schema context file location
    schema_context_path = project_root / "resources" / "schema" / "mapping_package_v3" / "models" / "context.jsonld"
    
    # If context.jsonld doesn't exist in schema directory, skip (it should be generated by Makefile)
    if not schema_context_path.exists():
        logger.warning(mssdk_config.MSSDK_LOGGING_MESSAGE_FORMAT.format(
            package_source=mapping_package_folder_path,
            message=f"context.jsonld not found in schema directory at {schema_context_path}. "
